...
export const useTransactionData = (userId) => {
  ...
  // adicionado funçaõ de delete transaction, para o delete da transação
  const deleteTransaction = async (transactionId) => {
    console.log('Tentando excluir transação:', { userId, transactionId });
    setLoading(true);
    setError('');
    ...
    try {
      ...
      const url = `/api/transactions/${userId}/${encodedTransactionId}`;
      console.log('URL da requisição DELETE:', url);
      
      const result = await apiConnection.delete(url);
